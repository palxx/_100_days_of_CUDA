...
__global__ void gpu_kernel_topdown(CSRgraph csrGraph, unsigned int* level, unsigned int* newVertexVisited, unsigned int currLevel){
...
__global__ void gpu_kernel_bottomup(CSRgraph csrGraph, unsigned int* level, unsigned int* newVertexVisited, unsigned int currLevel){
...
void bfs_gpu_normal(CSRgraph csrGraph, unsigned int srcVertex, unsigned int* level){
  ...
  if(currLevel == 1){
    gpu_kernel_topdown<<<numBlocks, numThreadsperblock>>>(csrGraph_d, level_d, newVertexVisited_d, currLevel);
  } else
  {
  gpu_kernel_bottomup<<<numBlocks, numThreadsperblock>>>(csrGraph_d, level_d, newVertexVisited_d, currLevel);
  }
