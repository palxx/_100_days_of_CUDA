...
def compile_cuda_program():
    compile_command = ["nvcc", "cuda_day_11.cu", "-o", "cuda_day_11"]
...
def run_cuda_program(N, M):
    with open('cuda_day_11.cu', 'r') as file:
        content = file.read()
    ...
    with open('cuda_day_11.cu', 'w') as file:
        file.write(content)

    compile_cuda_program()
    result = subprocess.run(['./cuda_day_11'], capture_output=True, text=True)
